...
func TestPBFTNode_ConsensusFlow(t *testing.T) {
	...
	for i, id := range nodeIDs {
		...
		nodes[i] = tpbft.NewPBFTNode(id, peers)
	}
	...
	prePrepareMsg := &tpbft.ConsensusMessage{
		Type:           tpbft.MessageTypePrePrepare,
		View:           view,
		SequenceNumber: seq,
		Digest:         "block-hash-1",
		NodeID:         "node0",
	...
	prepareMsgs := make([]*tpbft.ConsensusMessage, 4)
	for i, id := range nodeIDs {
		prepareMsgs[i] = &tpbft.ConsensusMessage{
			Type:           tpbft.MessageTypePrepare,
			View:           view,
			SequenceNumber: seq,
			Digest:         "block-hash-1",
			NodeID:         id,
	...
	commitMsgs := make([]*tpbft.ConsensusMessage, 4)
	for i, id := range nodeIDs {
		commitMsgs[i] = &tpbft.ConsensusMessage{
			Type:           tpbft.MessageTypeCommit,
			View:           view,
			SequenceNumber: seq,
			Digest:         "block-hash-1",
			NodeID:         id,
