...
// normalizeConfig 对分层热点感知 TPBFT 参数做规范化。
func normalizeConfig(cfg Config) Config {
	...
	if cfg.CrossGroupPenaltyFactor <= 0 {
		cfg.CrossGroupPenaltyFactor = 0.5
	}
	cfg.GroupingStrategy = strings.ToLower(cfg.GroupingStrategy)
	if cfg.GroupingStrategy == "" {
		cfg.GroupingStrategy = "random"
	}
	return cfg
}
