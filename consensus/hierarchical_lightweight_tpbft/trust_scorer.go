...
// normalizeConfig 对分层轻量 TPBFT 参数做规范化。
func normalizeConfig(cfg Config) Config {
	...
	cfg.SubConsensusType = strings.ToLower(cfg.SubConsensusType)
	if cfg.SubConsensusType == "" {
		cfg.SubConsensusType = "pbft"
	}
	if cfg.RaftHeartbeatMs <= 0 {
		cfg.RaftHeartbeatMs = 50
	}
	if cfg.RaftElectionMs <= 0 {
		cfg.RaftElectionMs = 200
	}
	return cfg
}
