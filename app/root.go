...
func addModuleInitFlags(startCmd *cobra.Command) {
	...
	startCmd.Flags().Int("ibft-node-count", 32, "IBFT 验证者数量")
	startCmd.Flags().Float64("ibft-faulty-ratio", 0, "IBFT 模拟故障比例(0-1)")
	startCmd.Flags().Float64("ibft-base-latency-ms", 1, "IBFT 基础网络时延(ms)")
	startCmd.Flags().Float64("ibft-jitter-ms", 50, "IBFT 网络抖动范围(ms)")
	startCmd.Flags().Float64("ibft-timeout-ms", 150, "IBFT 单轮超时(ms)")
	startCmd.Flags().Int("ibft-message-bytes", 256, "IBFT 单消息字节数")
	startCmd.Flags().Int("ibft-max-rounds", 8, "IBFT 单高度最大轮次")
